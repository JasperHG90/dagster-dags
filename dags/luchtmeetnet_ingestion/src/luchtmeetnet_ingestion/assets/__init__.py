...
@asset(
    ...
    required_resource_keys={"luchtmeetnet_api"},
...
def air_quality_data(context: AssetExecutionContext) -> Output[pd.DataFrame]:
    ...
    df = pd.DataFrame(
        context.resources.luchtmeetnet_api.request(
            "measurements", context=context, request_params=rp
        )
    )
